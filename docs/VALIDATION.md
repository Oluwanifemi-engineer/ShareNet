...
- `Ipv4CodecTest` — checksum-correct IPv4/TCP/UDP codec
...
- [ ] Throughput: per-client and aggregate numbers on the shared single radio
...
- ICMP (ping) is dropped in tunnel mode — needs a raw/privileged socket
...
```bash
JAVA_HOME=~/jdk21 ./gradlew :app:testDebugUnitTest          # layer 1 (78 tests)
